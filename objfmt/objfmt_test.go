...
func TestClassDispatch(t *testing.T) {
	if _, err := Size(ClassSubtree, []byte{0}); !errors.Is(err, ErrClassNotRegistered) {
		t.Fatalf("subtree: %v, want ErrClassNotRegistered", err)
	}
	if _, err := Size(ClassBlock, []byte{0}); !errors.Is(err, ErrClassNotRegistered) {
		t.Fatalf("block: %v, want ErrClassNotRegistered", err)
	}
